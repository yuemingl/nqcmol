 ...
 * @author nqc
 */
public class OSS2_FCPotential extends Potential{
	public OSS2_FCPotential(){
		nativeUnit="Hartree";
		unit=nativeUnit;
		ParseParameters();
	};
	...
	public void setParam(String filename) {
		...
		try {
			...
			Logger.getLogger(OSS2_FCPotential.class.getName()).log(Level.SEVERE, null, ex);
		}
		}
		ParseParameters();
	}
	...
	double ChargeChargeInteraction(boolean isGrad){
		...
		for(int i=0; i<nAtom; i++ ){
			for(int j=i+1; j< nAtom;j++){
				...
				Vq+= temp1;
                //System.out.printf(" %f %f ++ Vq====== %12.8f\n",q[i],q[j],Vq);
				if(isGrad){ //for gradient
					for(int k=0;k<3;k++){
