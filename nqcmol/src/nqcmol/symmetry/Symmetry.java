...
public class Symmetry {
	...
	public static final int MAX_OPT_CYCLES_DEFAULT = 1000;
	...
	public void findSymmetryElements(boolean showInfo) throws SymmetryException {
		...
		if (showInfo) {
			System.err.println("Looking for the inversion center");
		}
		...
		if (showInfo) {
			...
			System.err.println("Looking for the planes of symmetry");
		}
		...
		if (showInfo) {
			reportAndResetCounters();
			System.err.println("Looking for infinity axis");
		}
		...
		if (showInfo) {
			reportAndResetCounters();
			System.err.println("Looking for C2 axes");
		}
		...
		if (showInfo) {
			reportAndResetCounters();
			System.err.println("Looking for higher axes");
		}
		...
		if (showInfo) {
			reportAndResetCounters();
			System.err.println("Looking for the improper axes");
		}
	...
	public PointGroup identifyPointGroup(boolean showInfo) {
		...
		for (i = 0; i < PointGroups.defaultPointGroups.length; i++) {
			//System.err.println("symmetryCode:'"+new
			...
			if (symmetryCodeS.equals(
				...
				if (PointGroups.defaultPointGroups[i].getCheck() == true) {
					last_matching = i;
					order_point_group=ORDER_POINT_GROUP[i]; //added by nqc
					matching_count++;
				} else {
					//if (showInfo) {
					//	System.err.println("It looks very much like " +
		...
		if (matching_count > 1) {
			if (showInfo) {
				...
				System.err.println("Matching groups are:");

				for (i = 0; i < PointGroups.defaultPointGroups.length; i++) {
					if ((symmetryCode.equals(
							PointGroups.defaultPointGroups[i].getSymmetryCode()) == false) &&
		...
		if (matching_count == 1) {
			if (showInfo) {
				System.err.println("It seems to be the " +
	 ...
	 */
	public boolean readCoordinates(Cluster mol) {
		atomsCount = mol.getNAtoms();
...
//		}
		...
		if (atoms == null) {
			...
			for (int atomIdx = 0; atomIdx < atomsCount; atomIdx++) {
				...
				atoms[atomIdx].type = mol.getAtomicNumber(atomIdx);
				atoms[atomIdx].coord[0] = mol.getCoords(atomIdx * 3);
				atoms[atomIdx].coord[1] = mol.getCoords(atomIdx * 3 + 1);
				atoms[atomIdx].coord[2] = mol.getCoords(atomIdx * 3 + 2);
			}
		}
	...
	protected String buildSymmetryCode() {
		...
		if ((planesCount + normalAxesCount + improperAxesCount +
			...
			//System.err.println("Molecule has no symmetry elements\n");
		} else {
			//System.err.println("Molecule has the following symmetry elements:
			// ");
			if (inversionCentersCount > 0) {
				symmetryCode+="(i) ";
			}
			...
			if (normalAxesCounts[0] > 1) {
				symmetryCode+=Integer.toString(normalAxesCounts[0]);
				symmetryCode+="*(Cinf) ";
			}
			...
			for (i = getMaxAxisOrder(); i >= 2; i--) {
				if (normalAxesCounts[i] == 1) {
					symmetryCode+="(C";
					symmetryCode+=Integer.toString(i);
					symmetryCode+=") ";
				}

				if (normalAxesCounts[i] > 1) {
					symmetryCode+=Integer.toString(normalAxesCounts[i]);
					symmetryCode+="*(C";
					symmetryCode+=Integer.toString(i);
					symmetryCode+=") ";
				}
			...
			for (i = getMaxAxisOrder(); i >= 2; i--) {
				...
				if (improperAxesCounts[i] > 1) {
					symmetryCode+=Integer.toString(improperAxesCounts[i]);
					symmetryCode+="*(S";
					symmetryCode+=Integer.toString(i);
					symmetryCode+=") ";
				}
			...
			if (planesCount > 1) {
				symmetryCode+=Integer.toString(planesCount);
				symmetryCode+="*(sigma) ";
			}
		}
		//System.err.println(" symmetryCode= "+symmetryCode.toString());
		//this.symmetryCode=symmetryCode.toString();
		return symmetryCode;
	}
	...
	protected void sortSymmetryElements() {
		...
		//System.err.printf("planesCount= %d, normalAxesCount= %d, improperAxesCount= %d\n",planesCount,normalAxesCount,improperAxesCount);
		if (planesCount > 1) {
			//sorting.sort(planes, axesComparator);
			java.util.Arrays.sort(planes, axesComparator);
		}

		if (normalAxesCount > 1) {
			//sorting.sort(normalAxes, axesComparator);
			java.util.Arrays.sort(normalAxes, axesComparator);
		}

		if (improperAxesCount > 1) {
			//sorting.sort(improperAxes, axesComparator);
			java.util.Arrays.sort(improperAxes, axesComparator);
		}
	}
	...
	*/
	private int[] ORDER_POINT_GROUP={
		1,2,2,2,3,4,5,6,7,8, //for Cn
		4,6,8,10,12,14,16, //for Dn
		4,6,8,10,12,14,16, //for Cnv
		4,6,8,10,12,14,16, //for Cnh
		8,12,16,20,24,28,32, //for Dnh
		8,12,16,20,24,28,32, //for Dnd
		4,6,8, //for Sn
		12,24,24, //for T,Th,Td
		24,48, //for O,Oh
		4,//Cinfv
		8,//Dinfh
		60,120, //for I,Ih
	...
	int order_point_group=1;//added by nqc

	public int getPointGroupOrder(){//added by nqc
		return order_point_group;//added by nqc
	}
	 ...
	 * @param filename
	 */
	public static void CalculateSymmetry(Symmetry symmetry, Cluster mol,int verbose) {
		...
		try {
			if(verbose>=2)
				...
				symmetry.findSymmetryElements(false);
	...
	private int checkTransformQuality(SymmetryElement elem)
		...
		for (i = 0, max_r = 0; i < atomsCount; i++) {
			...
			if (r > getToleranceFinal()) {
				//System.err.println("distance to symmetric atom (" + r + ") is
...
//		}
	...
	private void findPlanes() throws SymmetryException {
		...
		//              else
		//              {
		//                      //System.err.println("planes ["+(planesCount - 1)+"]="+plane);
...
//						}
						...
						//System.err.println("rotation axis order (" + order +
						// ")");
...
//									}
	...
	private void reportAndResetCounters() {
		System.err.println("  " + statistic.totalExaminedCandidates +
				" candidates examined\n" + "  " + statistic.earlyRemovedCandidates +
				" removed early\n" + "  " + statistic.removedInitialMating +
	...
	private void reportAxes() {
		if (normalAxesCount == 0) {
			System.err.println("There are no normal axes in the molecule");
		} else {
			if (normalAxesCount == 1) {
				System.err.println("There is a normal axis in the molecule");
			} else {
				System.err.println("There are " + normalAxesCount +
			...
			for (int normalAxe = 0; normalAxe < normalAxesCount; normalAxe++) {
				if (normalAxes[normalAxe] != null) {
					System.err.print("" + normalAxe + " " +
							normalAxes[normalAxe].maxdev + " ");

					if (normalAxes[normalAxe].order == 0) {
						System.err.print("Inf ");
					} else {
						System.err.print(" " + normalAxes[normalAxe].order);
					}

					System.err.print(" (" + normalAxes[normalAxe].direction[0] +
	...
	private void reportImproperAxes() {
		...
		if (improperAxesCount == 0) {
			System.err.println("There are no improper axes in the molecule\n");
		} else {
			if (improperAxesCount == 1) {
				System.err.println("There is an improper axis in the molecule");
			} else {
				System.err.println("There are " + improperAxesCount +
			...
			for (i = 0; i < improperAxesCount; i++) {
				if (improperAxes[i] != null) {
					System.err.print("" + i + " " + improperAxes[i].maxdev);

					if (improperAxes[i].order == 0) {
						System.err.print(" Inf ");
					} else {
						System.err.print(" " + improperAxes[i].order);
					}

					System.err.print(" (" + improperAxes[i].direction[0] + "," +
	...
	private void reportInversionCenter() {
		if (inversionCentersCount == 0) {
			System.err.println("There is no inversion center in the molecule");
		} else {
			System.err.println("There in an inversion center in the molecule");
			System.err.println("     Residual                      Position");
			System.err.print("   " + inversionCenters[0].maxdev + " ");
			System.err.println("(" +
	...
	private void reportPlanes() {
		if (planesCount == 0) {
			...
			if (planesCount == 1) {
				...
				System.err.println("There are " + planesCount +
			...
			for (int plane = 0; plane < planesCount; plane++) {
				if (planes[plane] != null) {
					System.err.print("" + plane + " " + planes[plane].maxdev +
							" ");
					System.err.print("(" + planes[plane].normal[0] + "," +
							planes[plane].normal[1] + "," +
							planes[plane].normal[2] + ") ");
					System.err.println("" + planes[plane].distance + "");
				}
			}
		}
	}
	...
	private void setParams(SymmetryElement elem, double[] values) {
		//System.err.println("set params::"+values.length+" "+(elem.nparam-1));
		double[] tmp = new double[1];
