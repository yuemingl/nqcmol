...
public class TaskHarmonicSuperpositionApproximation extends Task{
	...
	public void ReadInputStructures() {
		try {
				...
				for (int i = 0; i < 50; i++) {
					...
					if ((j != -1) && (j <= args.length - 1 - 2)) {
						...
						while (scanner.hasNext()) {
							...
							if (!mol.Read(scanner, sFormatIn)) {
								break;
							}
							mol.setGroup(groupname);
							if(verbose>=1){
								xmllog.writeEntity("Cluster");
								xmllog.writeAttribute("id", Integer.toString(pop.size()));
	...
	@Option(name="-u",usage="unit of energy if applicant. Default is Hartree.",metaVar="STRING")
    String sUnit="Hartree";

	@Option(name="-auto",usage="auto classify clusters according to topologies")
    boolean bAuto=false;
